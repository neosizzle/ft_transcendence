...
const fetchUsers = async (reqUrl : string, navigate : NavigateFunction, setTotalElements : React.Dispatch<React.SetStateAction<number>>, setUsers : React.Dispatch<React.SetStateAction<User[] | null>>) => {
...
const Search: FunctionComponent = () => {
	...
	const [sort, setSort] = useState<Sort>({sortBy : "Ascending", sortOn : "username"});// current sort value
	...
	const [reqUrl, setReqUrl] = useState<string>(`${API_ROOT}/users?page=${currPage}&pageSize=${PAGE_SIZE}`); // request url
	...
	useEffect(() => {
		...
		Object.keys(filterPopulated).forEach(key => {
			if (key !== 'status')
			{
				if ( typeof key === 'string' )
					filterPopulated[key] = searchInput;
			}
		})
		...
		// append filterons
		if (Object.keys(filterPopulated).length > 0)
				newReqUrl += '&filterOn=';
	...
	<div onClick={(e)=>{
		...
		{
			const className : SVGAnimatedString = (e.target as Element).className as unknown as SVGAnimatedString;
		...
		if (currSelectedUser && !(e.target as Element).className.includes("action-menu"))
			setCurrSelectedUser(null);
	}}>
		<div className="flex flex-col items-center pb-20 sm:pb-0 overflow-auto">
			...
			{/* ActionMenu */}
			{
				currSelectedUser && dimensions.width && dimensions.width <= MOBILE_WIDTH ? 
				<ActionModal
