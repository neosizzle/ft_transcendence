...
const UserCard: FunctionComponent<UserCardProps> = ({user, setCurrSelectedUser, currSelectedUser, setOpenAlert, setAlertMsg}) => {
				...
				<div className="grid grid-cols-12 gap-4">
					...
					<div className="col-span-9 sm:col-span-6">
						<div className="grid grid-rows-2 sm:grid-rows-3 grid-flow-col gap-1">
							<div>
								<Link to={`/users/profile/${user.id}`}><span className="font-medium">{user?.username}</span> <span className="text-xs font-light">{user?.intraName}</span></Link>
							</div>
					...
					<div className="hidden sm:block col-span-3 h-full flex justify-end items-end">
							<div className={`flex justify-end ${user.id == currSelectedUser?.id ? "relative" : "" }`}>
								<button onClick={()=>{
									setCurrSelectedUser(user)
								}}>
									<svg xmlns="http://www.w3.org/2000/svg" className="menu-toggle h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
