...
const ActionModal: FunctionComponent<ActionModalProps> = ({user,  setAlertMsg, setOpenAlert, setCurrUser}) => {
		...
		<div className="relative w-full p-4 flex justify-center item-center">
			<div className="action-menu bg-white rounded-lg shadow dark:bg-gray-700">
				...
				<div className="p-6 mt-4">
					{/* View profile */}
					<button className="block text-left py-3 w-full px-2" disabled={loading} onClick={viewProfile}> View {user.username}&apos;s profile </button>

					{
						auth?.user?.id !== user.id ?
						<div>
							{/* Join game */}
							<button className="block text-left py-3 w-full px-2" disabled={loading} onClick={joinGame}> Join {user.username}&apos;s game </button>
							{/* Add friend */}
							<button className="block text-left py-3 w-full px-2" disabled={loading} onClick={addFriend}> Add {user.username} as friend </button>
							{/* Message user */}
							<button className="block text-left py-3 w-full px-2" disabled={loading} onClick={messageUser}> Message {user.username} </button>
							{/* Block User */}
							<button className="block text-left py-3 w-full px-2" disabled={loading} onClick={blockUser}> Block / Unblock {user.username} </button>
						</div> :
						null
					}
