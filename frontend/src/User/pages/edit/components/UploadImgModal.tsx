...
const DragNDrop:FunctionComponent<DragNDropProps> = ({setCurrFileName, setCurrFileSrc, currFileSrc}) => {
...
const UploadImgModal: FunctionComponent<UploadImgModalProps>= ({setOpenUploadModal, setCurrAvatar}) => {
		...
		<div className="relative w-full p-4 flex justify-center item-center">
			<div className="w-full sm:w-3/4 relative bg-white rounded-lg shadow dark:bg-gray-700">
				...
				<div className="p-6 text-center">
					...
					<div className="w-full mt-6">
						<label
							...
							<DragNDrop setCurrFileName={setCurrFileName} setCurrFileSrc={setCurrFileSrc} currFileSrc={currFileSrc}/>
						</label>
						
						{/* Filename */}
						<div className="flex justify-center w-full">
							<div className="md:w-2/3">
								<input disabled className="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full my-4 py-2 px-4 text-gray-700 leading-tight" type="text" placeholder={currFileName || "No file uploaded"}/>
							</div>
						</div>
					</div>
				...
				<button onClick = {()=>{
					if (loading) return;
					setOpenUploadModal(false)
				}} type="button" className={`${loading ? "opacity-50" : ""} text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600`}>Cancel</button>
				</div>
			</div>
		</div>
	</div>
