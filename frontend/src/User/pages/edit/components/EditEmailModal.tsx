...
const EditEmailModal: FunctionComponent<EditEmailModalProps> = ({
  setOpenEditEmailModal,
  setNewEmail,
}) => {
  ...
  const sendOtp = () => {
    auth_net_post(`${otpEndpoint}/request`, { email: currEmail }).then(
      (data) => {
  ...
  const verifyOtp = async (otp: string) => {
    setLoading(true);
    const res = await auth_net_post(`${otpEndpoint}/verify`, {
      check: currEmail,
      key: otpKey,
      otp,
    });
    ...
    <div className="fixed top-0 left-0 right-0 z-50 h-full overflow-x-hidden overflow-y-auto md:inset-0  bg-gray-300/50">
      <div className="relative w-full p-4 h-full flex justify-center item-center">
        ...
        <div className="relative bg-white rounded-lg h-3/4 shadow dark:bg-gray-700">
          {/* X Button */}
          <button
            onClick={() => setOpenEditEmailModal(false)}
            type="button"
          ...
          <div className="p-6 h-full  grid grid-rows-6 text-center gap-4">
            {!currSlide ? (
              ...
              <div className="row-span-5 sm:w-96 pt-10">
                ...
                <div>
                  <input
                    className={`bg-white appearance-none border-2 mt-2 ${
                      emailErr ? "border-red-200" : "border-gray-200"
                    } rounded  py-2 px-4 text-gray-700 leading-tight`}
                    type="text"
                    value={currEmail || ""}
              ...
              <div className="row-span-5 sm:w-96 p-5 pt-10">
                ...
                />
                {otpErr ? (
                  <span className="block text-sm text-red-700">{otpErr}</span>
                ) : null}
                {otpSuccess ? (
