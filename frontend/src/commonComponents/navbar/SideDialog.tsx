    ...
    >
      ...
      >
        ...
        <div className="grid grid-rows-6 grid-flow-col gap-4 justify-center items-center">
          ...
          {auth?.user ? (
            <div onClick={() => setOpenMenu(false)} className="w-full">
              <Link
                to="/logout"
            ...
            <div onClick={() => setOpenMenu(false)} className="w-full">
              <Link to="/login" className="block px-6 py-2 text-sm font-medium">
                Login
              </Link>
            </div>
          )}
          <div onClick={() => setOpenMenu(false)} className="w-full">
            <Link to="/chat" className="block px-6 py-2 text-sm font-medium">
              Chat
            </Link>
          </div>
          <div onClick={() => setOpenMenu(false)} className="w-full">
          ...
          <div onClick={() => setOpenMenu(false)} className="w-full">
            <Link to="/game" className="block px-6 py-2 text-sm font-medium">
              Game
            </Link>
          </div>
        </div>
