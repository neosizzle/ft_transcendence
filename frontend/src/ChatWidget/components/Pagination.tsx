...
const BackIcon = () => {
	return <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 lg:h-4 lg:w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
const PaginationBtn: FunctionComponent<PaginationBtnProps>  = ({children, onClick, borderless, active}) => {
	return <button onClick={onClick} className={`${active ? "bg-slate-500 text-white" : "bg-transparent text-slate-700"} font-semibold text-xs py-1 px-2 lg:text-base lg:py-2 lg:px-4 ${!borderless ? "border border-slate-500" : "border-0"} rounded`}>
		{children}
	</button>
}
...
const Pagination: FunctionComponent<PaginationProps> = ({setCurrPage, currPage, totalElements, pageSize}) => {
	...
	const [displayedPages, setDisplayedPages] = useState<number[]>([...Array(MAX_DISPLAY)].fill(MAX_INT));
