  ...
  describe("Friends", () => {
    ...
    it(`/friends (GET) filtering`, () => {
      return pactum
        .spec()
        .get(
          `/friends?page=1&pageSize=30&filterOn=avatar,status&filterBy=asdf.avatar.jpg,OFFLINE`
        )
        .withHeaders({
          Authorization: "Bearer $S{token}",
        })
    ...
    it(`/friends (GET) filtering 2`, () => {
      return pactum
        .spec()
        .get(
          `/friends?page=1&pageSize=30&filterOn=reqStatus,reqStatus&filterBy=PENDING,APPROVED`
        )
        .withHeaders({
          Authorization: "Bearer $S{token}",
        })
