...
const AddMinutesToDate = (date : Date, minutes : number) : Date => new Date(date.getTime() + minutes * MS_TO_MINS);

// encrypts a raw json object
const genEncKey = (id : number, model : OTP, check : string, secret : string) : string =>
{
	const res : OTPKey = {
		id,
		check,
...
export class OtpService {
	...
	async verifyOtp(dto : OtpVerifyDto)
	{
		...
		try {
			...
			if (decodedKey.check != dto.check) throw new BadRequestException("Bad Check");
		} catch (error) {
			if (error.message === 'Malformed UTF-8 data') throw new BadRequestException("Bad Key");
		}
		...
		if (CryptoJS.SHA256(dto.otp).toString() != OTPRes.otp) throw new BadRequestException("OTP Invalid");
		...
		const now = new Date();
		if (OTPRes.exprationTime.getTime() < now.getTime()) throw new BadRequestException("OTP Expired");
