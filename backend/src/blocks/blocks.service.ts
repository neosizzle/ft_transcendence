 ...
 */
  const generateBlocksWhere = (listObj : ListObject, userId : number) : Prisma.BlockWhereInput => {
 ...
 */
 const generateBlockPayload = (listObj : ListObject, userId : number) : Prisma.BlockFindManyArgs => {
	...
	{
		res.orderBy = [{blockee : {}}];
        res.orderBy[0].blockee[listObj.sortOn] = listObj.sortBy === "Ascending" ? "asc" : "desc"
	}
	return res;
}
...
export class BlocksService {
	...
	async getBlocks(user : User, query : ListQuery)
	{
        ...
        delete payload.take;
        delete payload.skip;
        const total_elements = await this.prisma.block.count(<Prisma.BlockCountArgs>payload)
	...
	async addBlock(user : User, dto : BlocksDto)
	{
        ...
        if (userToBlock.id == user.id) throw new BadRequestException("Unable to block yourself");
        ...
        const res = this.prisma.block.create({
            data : {
                blockerId : user.id,
                blockeeId : userToBlock.id
            }
	...
	async removeBlock(user : User, blockeeId : number)
	{
		...
		if (user.id === blockeeId) throw new BadRequestException("Cannot remove self");
