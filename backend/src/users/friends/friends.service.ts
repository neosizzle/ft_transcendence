 ...
 */
 ...
 const generateFilters = (listObj : ListObject, userType : number) : Prisma.FriendshipWhereInput => {
 ...
 */
 const generateFriendsWhere = (listObj : ListObject, userId : number) : Prisma.FriendshipWhereInput => {
    // specify only to search for users containing me
    const res : Prisma.FriendshipWhereInput = {};
    res.AND = []
    res.AND.push({OR : [{AND : [{userId : userId}, generateFilters(listObj, FRIEND_USER)]},
 ...
 */
 const generateFriendPayload = (listObj : ListObject, userId : number) : Prisma.FriendshipFindManyArgs => {
	...
	{
		res.orderBy = [{friend : {}}];
        res.orderBy[0].friend[listObj.sortOn] = listObj.sortBy === "Ascending" ? "asc" : "desc"
	}
	return res;
}
...
export class FriendsService {
    ...
    {
        // validate query
        const listObj = validateListquery(query, ['reqStatus', ...Object.keys(user)])
        if (listObj.error)
    ...
    async addFriend(user : User, dto : FriendsDto)
    {
        ...
        if (userToAdd.id == user.id) throw new BadRequestException("Unable to send friend request to yourself");
        ...
        if (existingFriendship) throw new BadRequestException("Already added friend");
    ...
    async removeFriend(user : User, friendId : number)
    {
        ...
        if (user.id === friendId) throw new BadRequestException("Cannot remove self");
    ...
    async modifyFriend(user : User, id : number, dto : FriendsPatchDto)
    {
        ...
        if (user.id === dto.friendId) throw new BadRequestException("Cannot modify self");
        ...
        if (!Object.values(FriendshipStatus).find((e) => e === dto.reqStatus)) throw new BadRequestException("Bad request status")
